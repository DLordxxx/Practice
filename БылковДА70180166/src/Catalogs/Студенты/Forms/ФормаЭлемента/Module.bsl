...
Функция СеместрПриИзмененииНаСервере(Значение) 
    Если Значение = Перечисления.Семестры.Первый ИЛИ Значение = Перечисления.Семестры.Второй Тогда  
        Возврат Перечисления.Курсы.Первый; // Возвращаем курс
    ...
    КонецЕсли;	
    Возврат Неопределено; // Если значение семестра не подходит
КонецФункции

&НаКлиенте
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)

    Если НЕ ЗначениеЗаполнено(Объект.ЛичныйИндекс) Тогда
        Сообщить("Необходимо заполнить личный индекс студента.");
        Отказ = Истина;
        Возврат;   
	ИначеЕсли  НЕ ЗначениеЗаполнено(Объект.СтуденческийБилет) Тогда
        Сообщить("Необходимо заполнить личный индекс студента.");
        Отказ = Истина;
        Возврат;  
    КонецЕсли;
