...
Функция ПолучитьДисциплиныИзОснования(СсылкаНаОснование)
    // Если основание не указано, возвращаем пустой массив
    Если СсылкаНаОснование = Неопределено Тогда
        Возврат Новый Массив;
    КонецЕсли;
    
    // Если это не ИдентификацияАкадемическихЗадолженностей, возвращаем пустой массив
...
Процедура ДисциплинаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
    ...
    // Получаем информацию о факультете студента для дополнительной фильтрации
    ФакультетСтудента = ПолучитьФакультетСтудентаНаСервере();
    
    // Добавляем отбор по папкам (факультетам)
    ПараметрыФормы.Вставить("ОтборПоРодителю", Новый Структура("Родитель", ФакультетСтудента));
    
    // Открываем форму выбора с отбором
...
// Обработчик перед записью для дополнительной проверки
&НаКлиенте
