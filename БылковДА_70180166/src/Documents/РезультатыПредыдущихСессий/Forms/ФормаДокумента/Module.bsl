Процедура ЗаполнитьТабличнуюЧастьСлучайнымиДаннымиНаСервере()
    ...
    // Проверка на первокурсника (дублируем проверку для удобства)
    Запрос = Новый Запрос;
    Запрос.Текст = 
    "ВЫБРАТЬ
    ...
    // Остальной код заполнения табличной части
...
Процедура ПроверкаПроведенияНаСервере(Отказ, ПараметрыПроведения)
    ...
    // Проверка на пустую табличную часть
    Если Объект.ХарактеристикаСтудента.Количество() = 0 Тогда
        Отказ = Истина;
        ВызватьИсключение "Табличная часть 'ХарактеристикаСтудента' не содержит данных!";
    КонецЕсли;
    
    // Проверка на первокурсника
    Если НЕ ЗначениеЗаполнено(Объект.Студент) Тогда
        Отказ = Истина;
    ...
    // Дополнительная проверка на наличие задолженностей
    Если НетАкадемическихЗадолженностей() Тогда
        Отказ = Истина;
        ВызватьИсключение "Академические задолженности у студента отсутствуют!";
    КонецЕсли;
